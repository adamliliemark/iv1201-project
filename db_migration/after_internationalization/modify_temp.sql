...
insert into users
select name as first_name, surname as last_name, ssn, email, password from person 
where email is not null
and password is not null;

insert into unmigrated_person
select * from person where email is null or password is null;

insert into authorities (user_email, authority)
select email, "ROLE_USER" from person where role_id = 1 and email is not null and password is not null;

insert into authorities (user_email, authority) 
select email as user_email, "ROLE_ADMIN" from person where role_id = 2 and email is not null and password is not null;
...
insert into unmigrated_competence_profile(competence_profile_id, person_id, competence_name, years_of_experience)
select competence_profile_id, competence_profile.person_id, competence.name, years_of_experience from competence_profile, competence,unmigrated_person
